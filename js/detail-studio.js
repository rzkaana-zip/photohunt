...
class StudioApp {
    ...
    getCurrentUser() {
        return this.db.getUser('user-001') || {
            id: 'user-001',
            name: 'Guest User'
        };
    }
    ...
    loadStudioData() {
        try {
            const studio = this.db.getStudio(this.currentStudioId);
            
            if (!studio) {
                console.warn('Studio data missing or corrupt. Re-initializing DB...');
            ...
            Utils.showError('Terjadi kesalahan memuat data. Mohon refresh halaman.');
        }
    }
    ...
    showNotifications() {
        ...
        unreadNotifications.forEach(notif => {
            this.db.markNotificationAsRead(notif.id);
        });
        
        let message = 'Notifikasi Anda:\n\n';
