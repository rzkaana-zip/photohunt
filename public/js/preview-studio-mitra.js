document.addEventListener("DOMContentLoaded", () => {
    // 1. Ambil ID Studio dari URL Parameter (contoh: preview.html?id=studio_123)
    const urlParams = new URLSearchParams(window.location.search);
    const studioId = urlParams.get('id') || 'studio_001'; // Default ID untuk testing

    // 2. Referensi Elemen DOM (Wadah yang akan diisi)
    ...
    // 3. Setup Tombol Close
    btnClose.addEventListener("click", () => {
        // Logika kembali: bisa history back atau ke halaman dashboard
        window.history.back(); 
        // atau window.location.href = 'kelola-studio.html';
    });

    // 4. Panggil Fungsi Fetch Data
    ...
    function fetchStudioData(id) {
        // --- SIMULASI DATABASE ---
        // Nanti bagian ini diganti dengan: fetch(`/api/studios/${id}`)
        
        console.log(`Mengambil data untuk ID: ${id}...`);

        // Simulasi delay network (supaya kelihatan loadingnya)
        setTimeout(() => {
            // Ini adalah bentuk JSON standard yang biasa dikirim Backend
            const mockDatabaseResponse = {
                ...
                packages: [
                    ...
                    {
                        name: "Paket Group",
                        description: "Seru-seruan bareng geng",
                        duration: "45 Menit",
                        pax: "5 orang",
                        price: 100000
                    }
            ...
            // Jika data ditemukan, render ke HTML
    ...
    function renderStudio(data) {
        ...
        // B. Render Rating (Format Bintang)
        elRating.innerHTML = `⭐ <strong>${data.rating}</strong> (${data.review_count} ulasan)`;

        // C. Render Fasilitas (Looping Array)
        elFacilities.innerHTML = ''; // Bersihkan loading/placeholder
        if (data.facilities.length > 0) {
            data.facilities.forEach(facility => {
                const chip = document.createElement("span");
                chip.className = "mitra-chip"; // Pastikan class ini ada di CSS
